...
redis:setex(th3boss..'welcom_ph:witting'..msg.from.id,300,true)
...
👨🏽‍🔧]]
return tdcli.sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, "./data/photo/th3boss.jpg",msg_welcom,dl_cb,nil)
end
end

if matches[1] == 'اذاعه عام بالتوجيه' and is_sudo(msg) then		
if not we_sudo(msg) and redis:get(th3boss..'lock_brod') then 
...
redis:del(th3boss..'group:add'..matches[2]) 
redis:srem(th3boss..'group:ids',msg.to.id)
redis:del(th3boss..'group:name'..matches[2]) 
redis:del(th3boss..'lock_link'..matches[2]) 
redis:del(th3boss..'lock_id'..matches[2]) 
redis:del(th3boss..'lock_spam'..matches[2]) 
redis:del(th3boss..'lock_webpage'..matches[2]) 
redis:del(th3boss..'lock_markdown'..matches[2]) 
redis:del(th3boss..'flood'..matches[2]) 
redis:del(th3boss..'lock_bots'..matches[2]) 
redis:del(th3boss..'mute_forward'..matches[2]) 
redis:del(th3boss..'mute_contact'..matches[2]) 
redis:del(th3boss..'mute_location'..matches[2]) 
redis:del(th3boss..'mute_document'..matches[2]) 
redis:del(th3boss..'mute_keyboard'..matches[2]) 
redis:del(th3boss..'mute_game'..matches[2]) 
redis:del(th3boss..'mute_inline'..matches[2]) 
redis:del(th3boss..'num_msg_max'..matches[2]) 
redis:del(th3boss..'admins:'..matches[2])
redis:del(th3boss..'whitelist:'..matches[2])
redis:del(th3boss..'owners:'..matches[2])
...
if msg.to.type == 'channel' or msg.to.type == 'chat' and redis:get(th3boss..'group:add'..msg.to.id) then

if matches[1] == 'شحن' and matches[2] and is_sudo(msg) then
			if tonumber(matches[2]) > 0 and tonumber(matches[2]) < 1001 then
				local extime = (tonumber(matches[2]) * 86400)
				redis:setex(th3boss..'ExpireDate:'..msg.to.id, extime, true)
				if not redis:get(th3boss..'CheckExpire::'..msg.to.id) then
					redis:set(th3boss..'CheckExpire::'..msg.to.id,true)
				end
tdcli.sendMessage(msg.to.id, msg.id_, 1, '❖￤ تم شحن الاشتراك الى `'..matches[2]..'` يوم   ... 👍🏿', 1, 'md')
tdcli.sendMessage(SUDO, 0, 1, '❖￤ تم شحن الاشتراك الى `'..matches[2]..'` يوم   ... 👍🏿\n❖￤ في مجموعه ::  '..msg.to.title, 1, 'md')

			else
tdcli.sendMessage(msg.to.id, msg.id_, 1, '❖￤ عزيزي المطور ✋🏿\n👨🏻‍❖￤ شحن الاشتراك يكون ما بين يوم الى 1000 يوم فقط 🍃', 1, 'md')
			end
		end
...
if matches[1]:lower() == 'الاشتراك' and matches[2] then 
		if matches[2] == '1' then
			...
			redis:setex(th3boss..'ExpireDate:'..msg.to.id, timeplan1, true)
			if not redis:get(th3boss..'CheckExpire::'..msg.to.id) then
				redis:set(th3boss..'CheckExpire::'..msg.to.id,true)
			end
				tdcli.sendMessage(SUDO, msg.id_, 1, '❖￤ تم تفعيل الاشتراك  👍🏿\n📆¦  الاشتراك : `30 يوم`  *(شهر)*', 1, 'md')
				tdcli.sendMessage(msg.to.id, 0, 1, '❖￤ تم تفعيل الاشتراك   👍🏿\n📆¦  الاشتراك : `30 يوم`  *(شهر)*', 1, 'md')
		end
		if matches[2] == '2' then
			local timeplan2 = 7776000
			redis:setex(th3boss..'ExpireDate:'..msg.to.id,timeplan2,true)
			if not redis:get(th3boss..'CheckExpire::'..msg.to.id) then
			redis:set(th3boss..'CheckExpire::'..msg.to.id,true)
			end
		...
		if matches[2] == '3' then
			redis:set(th3boss..'ExpireDate:'..msg.to.id,true)
			if not redis:get(th3boss..'CheckExpire::'..msg.to.id) then
				redis:set(th3boss..'CheckExpire::'..msg.to.id,true)
			end
...
if matches[1]:lower() == 'الاشتراك'  and is_mod(msg) and not matches[2] then
			local check_time = redis:ttl(th3boss..'ExpireDate:'..msg.to.id)
...
function pre_process(msg)
	...
	local chex = redis:get(th3boss..'CheckExpire::'..msg.to.id)
	local exd = redis:get(th3boss..'ExpireDate:'..msg.to.id)
	if chex and not exd and not we_sudo(msg) and not is_sudo(msg) then
	local text3 = '❖￤ انتهى الاشتراك في احد المجموعات ✋🏿\n👨🏾‍❖￤ المجموعه : `'..msg.to.title..'`🍃\n❖￤ ايدي : '..msg.to.id
			local text4 = '❖￤ انتهى الاشتراك البوت✋🏿\n❖￤  سوف اغادر المجموعه فرصه سعيده 👋🏿\n👨🏾‍❖￤  او راسل المطور للتجديد @th3boss 🍃'
