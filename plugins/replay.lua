...
if r=="المشتركين" and is_sudo(msg) and msg.to.type == 'pv'  then
  local users = '❖￤ عدد المشتركين  `'..redis:scard(th3boss..'users')..'` *مشترك في البوت *🍃'
return users
end

if r=="/start" and msg.to.type == 'pv'  then
 redis:sadd(th3boss..'users',msg.from.id)
...
redis:setex(th3boss..'namebot:witting'..msg.from.id,300,true)
...
if not we_sudo(msg) then return "❖￤️هذا الاوامر للمطور الاساسي فقط 🛠" end
redis:setex(th3boss..'addrd_all:'..msg.from.id,300 , true)
redis:del(th3boss..'allreplay:'..msg.from.id)
return "❖￤ _حسننا الان ارسل كلمة الرد العام _🍃\n"
end
------------------------------------------------------
if msg.text then
if redis:get(th3boss..'namebot:witting'..msg.from.id) then --- استقبال اسم البوت
redis:del(th3boss..'namebot:witting'..msg.from.id)
redis:set(th3boss..'bot:name',msg.text)
reload_plugins( )
return "❖￤ تم تغير اسم البوت  ✋🏿\n❖￤ الان اسمه `"..check_markdown(r).."` 🍃"
end

if redis:get(th3boss..'addrd_all:'..msg.from.id) then -- استقبال الرد لكل المجموعات
if not redis:get(th3boss..'allreplay:'..msg.from.id) then-- استقبال كلمه الرد لكل المجموعات
redis:setex(th3boss..'allreplay:'..msg.from.id, 300 , msg.text )
...
-------------------------------------------------------------
if redis:get(th3boss..'addrd:'..msg.from.id) then -- استقبال الرد للمجموعه فقط
if not redis:get(th3boss..'replay1'..msg.from.id) then  -- كلمه الرد
redis:setex(th3boss..'replay1'..msg.from.id,300,msg.text)
return "❖￤ _شكرأ لك 😻_\n❖￤ _الان ارسل جواب الرد _✔️" 
end
...
--------------------------------------
if redis:get(th3boss..'delrd:'..msg.from.id) then
redis:del(th3boss..'delrd:'..msg.from.id)

if not redis:hget(th3boss..'replay:'..msg.to.id,msg.text) then
...
-------------------------------------
if redis:get(th3boss..'delrdall:'..msg.from.id) then
redis:del(th3boss..'delrdall:'..msg.from.id)
if not redis:hget(th3boss..'replay:all',msg.text) then
return '🗯هذا الرد ليش مضاف في قائمه الردود 🛠'
else
...
if not is_owner(msg) then return"♨️ للمدراء فقط ! 🛠" end
local names = redis:hkeys(th3boss..'replay:all')
for i=1, #names do
...
if not is_owner(msg) then return"♨️ للمدراء فقط ! 🛠" end
redis:set(th3boss..'delrdall:'..msg.from.id,true)
return "❖￤ حسننا عزيزي  ✋🏿\n❖￤ الان ارسل الرد لمسحها من  المجموعات 🍃"
end

if r== 'مسح رد' then
if not is_owner(msg) then return"♨️ للمدراء فقط ! 🛠" end
redis:set(th3boss..'delrd:'..msg.from.id,true)
...
if r=="اضف رد" then
if not is_owner(msg) then return"♨️ للمدراء فقط ! 🛠" end
redis:setex(th3boss..'addrd:'..msg.from.id,300 , true)
redis:del(th3boss.."replay1"..msg.from.id)
...
----------------------------------------------
if redis:hget(th3boss..'replay:all', r) then
return check_markdown(redis:hget(th3boss..'replay:all', r))
elseif redis:hget(th3boss..'replay:'..msg.to.id, r) then
return check_markdown(redis:hget(th3boss..'replay:'..msg.to.id, r))
