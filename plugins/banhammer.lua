...
if msg.to.type ~= 'pv' and redis:get(th3boss..'group:add'..msg.to.id)  then
...
--redis:hset(th3boss..'username:'..matches[2], 'username', user_name)
redis:sadd(th3boss..'banned:'..msg.to.id,matches[2])
kick_user(matches[2], msg.to.id)
return tdcli.sendMessage( msg.to.id, "", 0, '❖￤ الايدي  ⇔  *('..matches[2]..')*\n❖￤ _ تم حظره ✔️_', 0, "md")
end
if matches[2] and string.match(matches[2], '@[%a%d_]') then
...
--redis:hset(th3boss..'username:'..matches[2], 'username', user_name)
redis:srem(th3boss..'banned:'..msg.to.id,matches[2])
channel_unblock(msg.to.id, matches[2])
...
--redis:hset(th3boss..'username:'..matches[2], 'username', user_name)
redis:sadd(th3boss..'is_silent_users:'..msg.to.id,matches[2])
return tdcli.sendMessage(msg.to.id, "", 0, '❖￤ الايدي  ⇔  *('..matches[2]..')*\n❖￤ _ تم كتمه ✔️_', 0, "md")
end
if matches[2] and string.match(matches[2], '@[%a%d_]') then
...
--redis:hset(th3boss..'username:'..matches[2], 'username', user_name)
redis:srem(th3boss..'is_silent_users:'..msg.to.id,matches[2])
...
if #redis:smembers(th3boss..'banned:'..msg.to.id) ==0 then
return "*❖￤ لا يوجد مستخدمين محظورين  *"
end
redis:del(th3boss..'banned:'..msg.to.id)
----------------------------------------
...
if #redis:smembers(th3boss..'is_silent_users:'..msg.to.id) ==0 then
return "*❖￤ لا يوجد مستخدمين مكتومين في المجموعه *"
end
redis:del(th3boss..'is_silent_users:'..msg.to.id)
return "*❖￤ تم مسح قائمه الكتم*"
end
if matches[2] == 'المميزين'  then
if #redis:smembers(th3boss..'whitelist:'..msg.to.id) ==0 then
...
if #redis:smembers(th3boss..'gban_users') ==0 then
return "*❖￤ لا يوجد مستخدمين محظورين عام في المجموعه *"
end
redis:del(th3boss..'gban_users')
...
--redis:hset(th3boss..'username:'..matches[2], 'username', user_name)
redis:sadd(th3boss..'gban_users',matches[2])
kick_user(matches[2], msg.to.id)
...
--redis:hset(th3boss..'username:'..matches[2], 'username', user_name)
redis:srem(th3boss..'gban_users',matches[2])
channel_unblock(msg.to.id, matches[2])
