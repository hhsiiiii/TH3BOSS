...
do 
local update = 25; --  رقم اصدار السورس 
...
for k, v in pairs( plugins_names( )) do  
v = string.match (v, "(.*)%.lua") 
tdcli.sendDocument(msg.chat_id_, msg.id_,0, 1, nil, "./plugins/"..v..".lua", '❖￤ الملف مقدم من قناه  الزعيم ❖￤ \n❖￤ تابع قناة السورس @lBOSSl\n👨🏽‍🔧', dl_cb, nil)
end 
else
if not plugin_exists(matches[2]) then 
return '❖￤ لا يوجد ملف بهذا الاسم .\n\n'
else 
tdcli.sendDocument(msg.chat_id_, msg.id_,0, 1, nil, "./plugins/"..matches[2]..".lua", '❖￤ الملف مقدم من قناه  الزعيم ❖￤ \n❖￤ تابع قناة السورس @lBOSSl\n👨🏽‍🔧', dl_cb, nil)
end end end
...
tdcli.sendDocument(msg.chat_id_, msg.id_,0, 1, nil, send_file, '❖￤ الملف مقدم من قناه  الزعيم ❖￤ \n❖￤ تابع قناة السورس @lBOSSl\n👨🏽‍🔧', dl_cb, nil)
end
