...
redis:hset(th3boss..'username:'..v.user.id, 'username', user_name)
redis:sadd(th3boss..'admins:'..msg.to.id,v.user.id)
end 
end
if k ==0 then
...
redis:hset(th3boss..'username:'..v.user.id, 'username', user_name)
redis:sadd(th3boss..'owners:'..msg.to.id,v.user.id)
end
end
end
...
if redis:sismember(th3boss..'owners:'..msg.to.id,msg.from.id)then
var = true
end
...
if redis:sismember(th3boss..'admins:'..msg.to.id,msg.from.id)then
var = true
end
...
local var = false
if redis:sismember(th3boss..'owners:'..chat_id,user_id)then
var = true
end
for v,user in pairs(_config.sudo_users) do
if user[1] == user_id then
...
local var = false
if redis:sismember(th3boss..'admins:'..chat_id,user_id)then
var = true
end
...
function is_whitelist(user_id, chat_id)
  local var = false
if redis:sismember(th3boss..'whitelist:'..chat_id,user_id)then
var = true
end
return var
end
...
function is_banned(user_id, chat_id)
  local var = false
if redis:sismember(th3boss..'banned:'..chat_id,user_id)then
var = true
end
return var
end
...
local var = false
if redis:sismember(th3boss..'is_silent_users:'..chat_id,user_id)then
var = true
end
return var
end
...
local var = false
if redis:sismember(th3boss..'gban_users',user_id)then
var = true
end
return var
end
...
local var = false
local list =redis:smembers(th3boss..'users'..msg.to.id)
if #list ~= 0 then
for i = 1, #list do
    if list[i] == text then
       var = true
...
local list = redis:smembers(th3boss..'banned:'..chat_id)
if #list==0 then 
return "❖￤*  لايوجد أعضاء محظورين *"
end
message = '❖￤_  قائمه الاعضاء المحظورين :_\n'
for k,v in pairs(list) do
local info = redis:hgetall(th3boss..'username:'..v)
...
local list = redis:smembers(th3boss..'is_silent_users:'..chat_id)
if #list==0 then 
...
local list = redis:smembers(th3boss..'gban_users')
if #list==0 then 
...
local list = redis:smembers(th3boss..'klmamn3'..msg.to.id)
if #list == 0 then 
...
local list = redis:smembers(th3boss..'group:ids')
if #list==0 then 
return  "❖￤ _لا يوجد مجموعات مفعله حاليا ✓_"
end
message = '❖￤ *قائمه المجموعات :*\n\n'
for k,v in pairs(list) do
local info = redis:get(th3boss..'group:name'..v)
if info then
...
return '❖￤ عدد المجموعات المفعلة  : `'..#list..'` 🍃'
 ...
 function warning(msg)
	local expiretime = redis:ttl(th3boss..'ExpireDate:'..msg.to.id)
...
redis:del(th3boss..'group:add'..msg.to.id) 
redis:srem(th3boss..'group:ids',msg.to.id)
redis:del(th3boss..'group:name'..msg.to.id) 
redis:del(th3boss..'lock_link'..msg.to.id) 
redis:del(th3boss..'lock_id'..msg.to.id) 
redis:del(th3boss..'lock_spam'..msg.to.id) 
redis:del(th3boss..'lock_webpage'..msg.to.id) 
redis:del(th3boss..'lock_markdown'..msg.to.id) 
redis:del(th3boss..'flood'..msg.to.id) 
...
redis:del(th3boss..'num_msg_max'..msg.to.id) 
redis:del(th3boss..'extimeadd'..msg.to.id) 
redis:del(th3boss..'CheckExpire::'..msg.to.id)
redis:del(th3boss..'admins:'..msg.to.id)
redis:del(th3boss..'whitelist:'..msg.to.id)
redis:del(th3boss..'owners:'..msg.to.id)
redis:del(th3boss..'klmamn3'..msg.to.id)
...
redis:del(th3boss..'mute_text'..msg.to.id) 
...
redis:del(th3boss..'welcome'..msg.to.id)
redis:del(th3boss..'replay'..msg.to.id) 
redis:del(th3boss..'lock_woring'..msg.to.id)
local names = redis:hkeys(th3boss..'replay:'..msg.to.id)
for i=1, #names do
...
if not is_sudo(msg) and not redis:get(th3boss..'lock_service') then
return '❖￤ _أنـت لـسـت الـمـطـور _ ⚙️'
end

if redis:get(th3boss..'group:add'..msg.to.id) then -- حصانه التحقق من المجموعه اذا كانت مفعله
...
if is_group(msg.from.id) ~= true then return is_group(msg.from.id) end -- حصانه التحقق من المجموعه
set_Creator(msg)
redis:set(th3boss..'group:add'..msg.to.id,true) 
redis:sadd(th3boss..'group:ids',msg.to.id)
redis:set(th3boss..'group:name'..msg.to.id,msg.to.title) 
redis:set(th3boss..'lock_link'..msg.to.id,true) 
redis:set(th3boss..'lock_id'..msg.to.id,true) 
...
redis:set(th3boss..'flood'..msg.to.id,true) 
...
redis:set(th3boss..'mute_game'..msg.to.id,true) 
redis:set(th3boss..'mute_inline'..msg.to.id,true) 
redis:set(th3boss..'num_msg_max'..msg.to.id,5) 
...
if not redis:get(th3boss..'group:add'..msg.to.id) then
...
redis:del(th3boss..'replay'..msg.to.id) 
redis:del(th3boss..'lock_woring'..msg.to.id)
redis:setex(th3boss..'extimeadd'..msg.to.id, 300 , true)
local names = redis:hkeys(th3boss..'replay:'..msg.to.id)
for i=1, #names do
redis:hdel(th3boss..'replay:'..msg.to.id,names[i])
end
...
if redis:sismember(th3boss..'klmamn3'..msg.to.id,word) then
return  "❖￤ _ الكلمه_ *"..word.."* _هي بالتأكيد من قائمه المنع✓_"
end
redis:sadd(th3boss..'klmamn3'..msg.to.id,word)
return  "❖￤ _ الكلمه_ *"..word.."* _تمت اضافتها الى قائمه المنع ✓_"
end

function unfilter_word(msg, word)
if redis:sismember(th3boss..'klmamn3'..msg.to.id,word) then
redis:srem(th3boss..'klmamn3'..msg.to.id,word)
...
local list = redis:smembers(th3boss..'admins:'..msg.to.id)
if #list==0 then 
...
local list = redis:smembers(th3boss..'owners:'..msg.to.id)
if #list == 0 then 
...
local list = redis:smembers(th3boss..'whitelist:'..msg.to.id)
if #list == 0 then
...
if redis:sismember(th3boss..'whitelist:'..arg.chat_id,data.id_) then
return tdcli.sendMessage(arg.chat_id, "", 0, '❖￤ العضو  ⇔ '..user_name..'\n❖￤ الايدي  ⇔  *('..data.id_..')*\n❖￤ _ انه بالتأكيد من عضو مميز ✓ _', 0, "md")
end
redis:hset(th3boss..'username:'..data.id_, 'username', user_name)
redis:sadd(th3boss..'whitelist:'..arg.chat_id,data.id_)
...
redis:hset(th3boss..'username:'..data.id_, 'username', user_name)
redis:sadd(th3boss..'owners:'..arg.chat_id,data.id_)
...
if redis:sismember(th3boss..'admins:'..arg.chat_id,data.id_) then
return tdcli.sendMessage(arg.chat_id, "", 0, '❖￤ العضو  ⇔ '..user_name..' \n❖￤الايدي  ⇔  *('..data.id_..')*\n❖￤_ انه بالتأكيد ادمن ✓_', 0, "md")
end
redis:hset(th3boss..'username:'..data.id_, 'username', user_name)
redis:sadd(th3boss..'admins:'..arg.chat_id,data.id_)
...
if not redis:sismember(th3boss..'owners:'..arg.chat_id,data.id_) then
return tdcli.sendMessage(arg.chat_id, "", 0, '❖￤ العضو  ⇔ '..user_name..' \n❖￤ الايدي  ⇔  *('..data.id_..')*\n❖￤ _انه بالتأكيد ليس مدير ✓_', 0, "md")
end
redis:hdel(th3boss..'username:'..data.id_, 'username', user_name)
redis:srem(th3boss..'owners:'..arg.chat_id,data.id_)
...
if not redis:sismember(th3boss..'admins:'..arg.chat_id,data.id_) then
return tdcli.sendMessage(arg.chat_id, "", 0, '❖￤ العضو  ⇔ '..user_name..' \n❖￤ الايدي  ⇔  *('..data.id_..')*\n❖￤ _انه بالتأكيد ليس ادمن ✓_', 0, "md")
end
redis:hdel(th3boss..'username:'..data.id_, 'username', user_name)
redis:srem(th3boss..'admins:'..arg.chat_id,data.id_)
...
if redis:sismember(th3boss..'banned:'..arg.chat_id,data.id_) then
return tdcli.sendMessage(arg.chat_id, "", 0, '❖￤ العضو  ⇔  '..user_name..' \n❖￤ الايدي  ⇔ * ('..data.id_..')*\n❖￤ _ تم بالتأكيد حظره ✓_', 0, "md")
end
redis:hset(th3boss..'username:'..data.id_, 'username', user_name)
redis:sadd(th3boss..'banned:'..arg.chat_id,data.id_)
...
if not redis:sismember(th3boss..'banned:'..arg.chat_id,data.id_) then
return tdcli.sendMessage(arg.chat_id, "", 0, '❖￤ العضو  ⇔  '..user_name..' \n❖￤ الايدي  ⇔ * ('..data.id_..')*\n❖￤ _ تم بالتأكيد الغاء حظره ✓_', 0, "md")
end
redis:hdel(th3boss..'username:'..data.id_, 'username', user_name)
redis:srem(th3boss..'banned:'..arg.chat_id,data.id_)
...
if redis:sismember(th3boss..'is_silent_users:'..arg.chat_id,data.id_) then
return tdcli.sendMessage(arg.chat_id, "", 0, '❖￤ العضو  ⇔  '..user_name..' \n❖￤ الايدي  ⇔ * ('..data.id_..')*\n❖￤ _ تم بالتأكيد كتمه ✓_', 0, "md")
end
redis:hset(th3boss..'username:'..data.id_, 'username', user_name)
redis:sadd(th3boss..'is_silent_users:'..arg.chat_id,data.id_)
return tdcli.sendMessage(arg.chat_id, "", 0, '❖￤ العضو  ⇔  '..user_name..' \n❖￤ الايدي  ⇔ * ('..data.id_..')*\n❖￤ _ تم كتمه ✓_', 0, "md")
end
...
if not redis:sismember(th3boss..'is_silent_users:'..arg.chat_id,data.id_) then
return tdcli.sendMessage(arg.chat_id, "", 0, '❖￤ العضو  ⇔  '..user_name..' \n❖￤ الايدي  ⇔ * ('..data.id_..')*\n❖￤ _ تم بالتاكيد الغاء كتمه ✓_', 0, "md")
end
redis:hdel(th3boss..'username:'..data.id_, 'username', user_name)
redis:srem(th3boss..'is_silent_users:'..arg.chat_id,data.id_)
...
redis:hset(th3boss..'username:'..data.id_, 'username', user_name)
redis:sadd(th3boss..'gban_users',data.id_)
kick_user(data.id_, arg.chat_id)
...
redis:hdel(th3boss..'username:'..data.id_, 'username', user_name)
redis:srem(th3boss..'gban_users',data.id_)
channel_unblock(arg.chat_id, arg.user_id)
...
.."\n❖￤ قفل التعديل :"..(redis:get(th3boss..'lock_edit'..msg.to.id) or 'false')
.."\n❖￤ قفل الروابط :"..(redis:get(th3boss..'lock_link'..msg.to.id) or 'false')
.."\n❖￤ قفل التاك :"..(redis:get(th3boss..'lock_tag'..msg.to.id) or 'false')
.."\n❖￤ قفل المعرفات :"..(redis:get(th3boss..'lock_username'..msg.to.id) or 'false')
.."\n\n❖￤ قفل التكرار :"..(redis:get(th3boss..'flood'..msg.to.id) or 'false')
.."\n❖￤ قفل الكلايش :"..(redis:get(th3boss..'lock_spam'..msg.to.id) or 'false')
.."\n❖￤ قفل الويب :"..(redis:get(th3boss..'lock_webpage'..msg.to.id) or 'false')
.."\n❖￤ قفل الماركدوان :"..(redis:get(th3boss..'lock_markdown'..msg.to.id) or 'false')
.."\n❖￤ قفل التثبيت :"..(redis:get(th3boss..'lock_pin'..msg.to.id) or 'false')
.."\n❖￤ قفل البوتات بالطرد :"..(redis:get(th3boss..'lock_bots_by_kick'..msg.to.id) or 'false')
.."\n❖￤ قفل البوتات :"..(redis:get(th3boss..'lock_bots'..msg.to.id) or 'false')
.."\n❖￤ عدد التكرار :"..(redis:get(th3boss..'num_msg_max'..msg.to.id) or 'false')
.."\n\n❖￤` اعدادات الوسائط:`"
.."\n\n❖￤ قفل المتحركه :"..(redis:get(th3boss..'mute_gif'..msg.to.id) or 'false')
.."\n❖￤ قفل الدردشه :"..(redis:get(th3boss..'mute_text'..msg.to.id) or 'false')
.."\n❖￤ قفل الانلاين :"..(redis:get(th3boss..'mute_inline'..msg.to.id) or 'false')
.."\n❖￤ قفل الالعاب :"..(redis:get(th3boss..'mute_game'..msg.to.id) or 'false')
.."\n❖￤ قفل الصور :"..(redis:get(th3boss..'mute_photo'..msg.to.id) or 'false')
.."\n❖￤ قفل الفيديو :"..(redis:get(th3boss..'mute_video'..msg.to.id) or 'false')
.."\n❖￤ قفل البصمات :"..(redis:get(th3boss..'mute_audio'..msg.to.id) or 'false')
.."\n\n❖￤ قفل الصوت :"..(redis:get(th3boss..'mute_voice'..msg.to.id) or 'false')
.."\n❖￤ قفل الملصقات :"..(redis:get(th3boss..'mute_sticker'..msg.to.id) or 'false')
.."\n❖￤ قفل الجهات :"..(redis:get(th3boss..'mute_contact'..msg.to.id) or 'false')
.."\n❖￤ قفل التوجيه :"..(redis:get(th3boss..'mute_forward'..msg.to.id) or 'false')
.."\n❖￤ قفل الموقع :"..(redis:get(th3boss..'mute_location'..msg.to.id) or 'false')
.."\n❖￤ قفل الملفات :"..(redis:get(th3boss..'mute_document'..msg.to.id) or 'false')
.."\n❖￤ قفل الاشعارات :"..(redis:get(th3boss..'mute_tgservice'..msg.to.id) or 'false')
.."\n❖￤ قفل الكيبورد :"..(redis:get(th3boss..'mute_keyboard'..msg.to.id) or 'false')
.."\n\n❖￤` اعدادات اخرى :`"
.."\n❖￤ تفعيل الترحيب :"..(redis:get(th3boss..'welcome'..msg.to.id) or 'false')
.."\n❖￤ تفعيل الردود :"..(redis:get(th3boss..'replay'..msg.to.id) or 'false')
.."\n❖￤ تفعيل التحذير :"..(redis:get(th3boss..'lock_woring'..msg.to.id) or 'false')
.."\n❖￤ تفعيل الايدي :"..(redis:get(th3boss..'lock_id'..msg.to.id) or 'false') 
.."\n❖￤ مطور الـبـوت :"..sudouser
.."\n❖￤ قناة الـسـورس : @lBOSSl \n👨🏽‍🔧"
