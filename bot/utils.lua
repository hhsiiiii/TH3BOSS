...
if  redis:get(th3boss..'group:add'..msg.to.id) then  -- حصانه اذا كانت المجموعه مفعله

if matches[1] == "ايدي" then
if not matches[2] and not msg.reply_id then
if redis:get(th3boss..'lock_id'..msg.to.id) then
...
redis:setex(th3boss..'waiting_link'..msg.from.id, 300,true)
...
return '📭¦ حسننا عزيزي  ✋🏿\❖￤¦ الان ارسل القوانين  للمجموعه 🍃'
end
...
rules = "*❖￤القوانين :*\n"..redis:get(th3boss..'rulse:msg'..msg.to.id)
end
return rules
end
...
redis:set(th3boss..'flood'..msg.to.id,matches[2]) 
...
redis:del(th3boss..'rulse:msg'..msg.to.id)
return "❖￤ _مرحبآ عزيزي_ \n❖￤ _تم حذف القوانين بنجاح_ ✓"
end
if matches[2] == "الترحيب"  then
if not redis:get(th3boss..'welcome:msg'..msg.to.id) then
return "❖￤ _اوه ☢ هنالك خطأ_ 🚸\n❖￤ _عذرا لا يوجد ترحيب ليتم مسحه_ ✓"
end
redis:del(th3boss..'welcome:msg'..msg.to.id)
return "❖￤ _مرحبآ عزيزي_ \n❖￤ _تم حذف الترحيب بنجاح_ ✓"
end
end 
if matches[1] == "مسح" and is_sudo(msg) then
if matches[2] == "المدراء" then
if #redis:smembers(th3boss..'owners:'..msg.to.id) ==0 then
...
return "📭¦ حسننا عزيزي  ✋🏿\❖￤¦ الان ارسل الاسم  للمجموعه 🍃"
end
...
return "📭¦ حسننا عزيزي  ✋🏿\❖￤¦ الان ارسل الوصف  للمجموعه 🍃"
end
...
if matches[2] == "الايدي" then
if not redis:get(th3boss..'lock_id'..msg.to.id) then
return "❖￤ _مرحبا عزيزي_\n❖￤ _الايدي بالتأكيد معطل_ ✓"
else
redis:del(th3boss..'lock_id'..msg.to.id)
return "❖￤ _مرحبا عزيزي_\n❖￤ _تم ايقاف امر الايدي_ ✓"
end
end
if matches[2] == "الترحيب" then
if not redis:get(th3boss..'welcome:get'..msg.to.id) then
...
if matches[2] == "التحذير" then
if not redis:get(th3boss..'lock_woring'..msg.to.id) then
...
return "📭¦ حسننا عزيزي  ✋🏿\❖￤¦ ارسل كليشه الترحيب الان 🍃"
end
if matches[1] == "الترحيب"  and is_mod(msg) then
if redis:get(th3boss..'welcome:msg'..msg.to.id)  then
return redis:get(th3boss..'welcome:msg'..msg.to.id)
else
return "❖￤ مرحباً عزيزي\n❖￤ نورت المجموعه \n❖￤ تابع : @lBOSSl \n💂🏼‍♀️"
end
end
...
redis:del(th3boss..'welcom:witting'..msg.from.id)
redis:set(th3boss..'welcome:msg'..msg.to.id,check_markdown(msg.text))
...
return "📭¦ تم تغير اسم المجموعه  ✋🏿\❖￤¦ الان اسمه `"..check_markdown(msg.text).."` 🍃"
end
--------------------------------------------------------------
if redis:get(th3boss..'about:witting'..msg.from.id) then --- استقبال الوصف
redis:del(th3boss..'about:witting'..msg.from.id)
...
redis:del(th3boss..'fwd:all'..msg.from.id)
...
tdcli.sendMessage(msg.to.id, 0, 0,'❖￤ تم اذاعه الى `'..redis:scard(th3boss..'users')..'` من المشتركين 👍🏿', 0)
end
---------------------------------------------------------------------------------
if redis:get(th3boss..'fwd:pv'..msg.from.id) then ---- استقبال رساله الاذاعه خاص
redis:del(th3boss..'fwd:pv'..msg.from.id)
local pv = redis:smembers(th3boss..'users')
for i = 1, #pv do
tdcli.sendMessage(pv[i], 0, 0, check_markdown(msg.text), 0)			
end
...
---------------------------------------------------------------------------------
if redis:get(th3boss..'fwd:groups'..msg.from.id) then ---- استقبال رساله الاذاعه خاص
redis:del(th3boss..'fwd:groups'..msg.from.id)
local groups = redis:smembers(th3boss..'group:ids')
for i = 1, #groups do
tdcli.sendMessage(groups[i], 0, 0, check_markdown(msg.text), 0)			
end
...
if redis:get(th3boss..'group:add'..msg.to.id) and redis:get(th3boss..'welcome:get'..msg.to.id) and msg.adduser ~= tonumber(our_id) then
if msg.adduser then
